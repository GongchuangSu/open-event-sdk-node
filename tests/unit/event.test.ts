...
import { buildEventCode, createEvent, createTypedEvent, EventCodes } from '../../src/event';

describe('event', () => {
  describe('buildEventCode', () => {
    it('should build correct event code', () => {
      expect(buildEventCode('kso.app_chat.message', 'create')).toBe('kso.app_chat.message.create');
      expect(buildEventCode('kso.app_chat', 'create')).toBe('kso.app_chat.create');
      expect(buildEventCode('kso.xz.app.group_chat', 'delete')).toBe('kso.xz.app.group_chat.delete');
    });
  });

  describe('createEvent', () => {
    it('should create event with correct properties', () => {
      const event = createEvent('kso.app_chat.message', 'create', 1704067200, '{"message": "hello"}');
  ...
  describe('EventCodes', () => {
    it('should have correct event code values', () => {
      ...
      expect(EventCodes.V7_APP_GROUP_CHAT_MEMBER_USER_CREATE).toBe('kso.xz.app.group_chat.member.user.create');
      expect(EventCodes.V7_APP_GROUP_CHAT_MEMBER_USER_DELETE).toBe('kso.xz.app.group_chat.member.user.delete');
      expect(EventCodes.V7_APP_GROUP_CHAT_MEMBER_ROBOT_CREATE).toBe('kso.xz.app.group_chat.member.robot.create');
      expect(EventCodes.V7_APP_GROUP_CHAT_MEMBER_ROBOT_DELETE).toBe('kso.xz.app.group_chat.member.robot.delete');
    });
  });
});
